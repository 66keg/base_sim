/*!
 * @file car_sim_node.cpp
 * @brief 四輪ロボットの2D運動学シミュレーション
 * @author Keiji Muro
 * @data 2018.11.13: ver0.0.1 新規作成
 */
#include "ros/ros.h"
...
{
	ros::init(argc, argv, "car_sim_node");
	ros::NodeHandle n("~");
	...
	tf::TransformBroadcaster tf_odom2base, tf_base2handle;

	ros::Rate loop_rate(1.0/CONTROL_PERIOD);

	double length;
	n.param("length", length, 0.5);
	...
	while (ros::ok())
	{
		g_handle_ang = g_cmd_w;
		g_odom_v = g_cmd_v;
		g_odom_w = tan(g_handle_ang) / length * g_cmd_v;
		...
		trans_link.header.stamp 			= current_time;
		trans_link.header.frame_id 		= "base_link";
		trans_link.child_frame_id 		= "handle_link";
		trans_link.transform.translation.x 	= length;
		trans_link.transform.translation.y 	= 0.0;
		trans_link.transform.translation.z 	= 0.0;
		trans_link.transform.rotation 		= tf::createQuaternionMsgFromYaw(g_handle_ang);
		tf_base2handle.sendTransform(trans_link);
